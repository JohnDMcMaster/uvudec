...
# One of my first Python programs...yum
...
def injectionTemplateFromDLL(dllFile, CSourceFile, configFile):
	'''Generate a .inj file from a file containing functions and a C source file''' 
	def processFunctionDecl(line):
		for i in range(0, len(symbols)):
			sym  = symbols[i]
			if sym is None:
				continue
			#print "%s find %s" (line, sym)
			if line.find(sym) >= 0:
				# Preserve ordinal
				#print 'line: %s, i: %d' % (line, i)
				outputLines[i] = line.split(';')[0]
				symbols[i] = None
				break
	
	
	import pefile
	#functionsRaw = open(exportsFile).read().split()
	pe = pefile.PE(dllFile)
	# If we are by ordinal, order must be preserved
	symbols = list()
	for sym in pe.DIRECTORY_ENTRY_EXPORT.symbols:
		symbols.append(sym.name)
		#print sym.name
	nSymbols = len(symbols)
	outputLines = [''] * nSymbols

	CSourceCode = open(CSourceFile).read()
	inFuncDecls = False
	for line in CSourceCode.split('\n'):
		line = line.strip()
		if inFuncDecls:
			# header for def func
			if line.find("--------------") >= 0:
				break
			processFunctionDecl(line)
		else:
			# func decl start area
			if line.find("// Function declarations") >= 0:
				inFuncDecls = True
		...
		for sym in symbols:
			if sym is not None:
				if not issuedWarning:
					print 'WARNING: could not match the following symbols:'
					issuedWarning = True
				print '\t%s' % sym
	
	dllFileSimple = os.path.basename(dllFile).split('.')[0]
	output = ''
	output += "DLL=%s\n" % dllFileSimple
	...
	print output
	if configFile:
		outputFile = open(configFile, "w")
		outputFile.write(output)

def isValidFunctionDeclaration(line):
	# A line that isn't fully there
	# // __int16 __userpurge APoorConditionFunction<ax>(int a1<ebx>, unsigned __int16 a2, wchar_t *a3, wchar_t *a4, int a5, int a6)
	return line.find("//") != 0 and line.find("<eax>") < 0 and line.find("<ebx>") < 0 and line.find("<ecx>") < 0 and line.find("<edx>") < 0		

def injectionInferface(configFile):
	# header place at top of each file
	copyright = """\
/*
 * Generated by uvinj.py version %s @ %s
 * uvinj.py copyright 2009 John McMaster <JohnDMcMaster@gmail.com>
 */


""" % (VERSION, time.strftime('%F %T'))
	...
	KEY_FUNCTION_VARIADIC = "variadic"
	...
	for line in infile:
		...
		# Assume ordinal line then
			...
			if not isValidFunctionDeclaration(line):
				msg = 'ERROR: cannot process line: %s' % line
				print msg
				errors.append(msg)
				failure = True
			elif line.find("extern") >= 0:
				variable = dict()
				...
				if len(argsRaw) == 0:
					...
					for argRaw in argsRawVector:
						...
						if argType == "":
							raise Exception('Empty arg type')
						print "Arg name: " + str(argName)
				...
				function[KEY_FUNCTION_VARIADIC] = variadic
	...
	if failure:
		...
		for error in errors:
			print error
		print "Previous failures, exiting"
		sys.exit(1)
	...
	def writeDefFile():
		...
		for variable in variables:
			...
			if len(variableName) >= namePad:
				namePad = len(variableName) + 1
		for function in functions:
			functionName = function[KEY_FUNCTION_NAME]
			if len(functionName) >= namePad:
				namePad = len(functionName) + 1
		...
		for variable in variables:
			...
			toWrite += "\t" + variableName.ljust(namePad) + "@" + variableOrdinal + "\n"

		for function in functions:
			functionName = function[KEY_FUNCTION_NAME]
			functionOrdinal = function[KEY_FUNCTION_ORDINAL]

			toWrite += "\t" + functionName.ljust(namePad) + "@" + functionOrdinal + "\n"
...
BOOL Init()
{
	...
	"""
		...
		toWrite += '	printf("Loading library %s\\n", szLibrary);\n'
		toWrite += '	' + globalLibraryHandle + ' = LoadLibrary(szLibrary);\n'
		toWrite += '	if( ' + globalLibraryHandle + ' == NULL )\n'
		toWrite += '	{\n'
		toWrite += '		printf("Failed to load library %s\\n", szLibrary);\n'
		toWrite += '		goto error;\n'
		toWrite += '	}\n'
					 ...
					 )
{
	...
	"""
		...
		def getFunctionDefinition(function):
			...
			if function[KEY_FUNCTION_VARIADIC]:
				toWrite += '\tprintf("%s is variadic...uh good luck\\n");\n' % functionName
...
def usage():
	print "uvinj [args] <file>"
	print 'Args:'
	print '--help: this message'
	print 'To generate template .inj from DLL and template C file:'
	print "\t--dll=<file>"
	print "\t--c-file=<file>"

def main():
	# extern int g_variable;
	# int __stdcall aFunction(int a1);
	configFile = None
	dllFile = None
	CSourceFile = None
	...
	if dllFile is not None or CSourceFile is not None:
		injectionTemplateFromDLL(dllFile, CSourceFile, configFile)
		if configFile:
			injectionInferface(configFile)
