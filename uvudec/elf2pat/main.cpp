...
static uv_err_t doConvert(std::string file)
{
	...
	printf_debug_level(UVD_DEBUG_SUMMARY, "doConvert: initializing engine...\n");
	...
	rc = disasm->get(file, output);
	if( UV_FAILED(rc) )
	{
		printf_error("Failed to doConvert!\n");
		return UV_DEBUG(UV_ERR_GENERAL);
	}
...
uv_err_t initProgConfig()
{
	...
	//Arguments
	g_config->m_configArgs.push_back(new UVDArgConfig(UVD_PROP_TARGET_FILE, 0, "input", "ELF library", 1, argParser, false));
...
uv_err_t uvmain(int argc, char **argv)
{
	...
	if( UV_FAILED(doConvert(g_targetFile)) )
	{
